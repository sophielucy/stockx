...
let translate (func_decls, stmts) =
  ...
  let fn = L.define_function "main" ftype the_module in
  ...
  let local_vars =
    let add_local map (t, n) =
      let local_var = L.build_alloca (ltype_of_typ t) n builder
      in StringMap.add n local_var map
    in

    let rec get_locals mylocals = function
        [] -> mylocals
      | [A.V_Assign (t, s)] -> get_locals [(t, s)] []
      | A.V_Assign (t, s) :: r -> get_locals ( (t, s) :: mylocals) r
